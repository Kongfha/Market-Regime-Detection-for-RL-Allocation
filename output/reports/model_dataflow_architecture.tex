\documentclass[11pt,a4paper]{article}
            ...
            \begin{itemize}[leftmargin=1.4em]
              \item Extraction: repo macro fetch pipeline with default preset \texttt{core}, using FRED-format identifiers plus series-specific fallbacks where needed.
            ...
            \begin{itemize}[leftmargin=1.4em]
              ...
              \item Processed outputs: \texttt{news\_events\_enriched.csv} and \texttt{news\_features\_weekly.csv}; current build leaves both empty because the raw Yahoo news snapshot is absent.
            ...
            \begin{itemize}[leftmargin=1.4em]
              \item Extraction: Hugging Face archive download when the optional benchmark dataset is refreshed.
              \item Raw schema: sentence, label
              \item Model use: benchmark only for sanity-checking the sentiment pipeline; not present in the current working tree.
            \end{itemize}
            ...
            \midrule
            \texttt{model\_state\_weekly\_price\_macro.csv} & Price features + macro features + weekly asset targets (625 rows, 75 columns) & Main historical regime training table and first RL table \\
            \texttt{model\_state\_weekly\_full.csv} & Price + macro + targets + nullable news features (625 rows, 90 columns) & Unified schema for integration code \\
            \texttt{model\_state\_weekly\_recent\_full.csv} & Recent rows where price, macro, and news all exist (currently 0 rows, 90 columns) & Multimodal demos, case studies, and explanation outputs once the raw news snapshot is refreshed \\
            \bottomrule
            \end{tabularx}
