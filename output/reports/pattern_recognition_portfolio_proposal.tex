...
Macro panel & Repo macro fetch pipeline & Multi-decade history, latest values mostly through March 2026 & Daily, weekly, monthly mixed & Regime context, macro stress, rates, inflation expectations, and slow macro confirmation \\
News metadata & Yahoo Finance via \texttt{get\_news()} & Not present in the current working snapshot & Event-driven & Optional recent sentiment, topic, relevance, and live-market context after refresh \\
Text sanity set & Financial PhraseBank & Not present in the current working snapshot & Sentence-level labeled text & Optional sentiment pipeline validation only; not the main trading dataset \\
\bottomrule
\end{tabularx}
\end{table}

Historical market data are collected through the \texttt{yfinance} interface\fncite{Ran Aroussi, \emph{yfinance} documentation}{https://ranaroussi.github.io/yfinance/index.html}. The macro panel is assembled by the repository fetch pipeline around FRED-format series identifiers and metadata\fncite{Federal Reserve Bank of St.~Louis, FRED API overview}{https://fred.stlouisfed.org/docs/api/fred/overview.html}. The optional text sanity set is based on Financial PhraseBank and related financial sentiment literature\fncite{Pekka Malo et al., \emph{Good Debt or Bad Debt: Detecting Semantic Orientations in Economic Texts}, 2013}{https://arxiv.org/abs/1307.5336}.

In this proposal, \emph{tradable assets} are the instruments that receive portfolio weights directly, namely SPY, TLT, and GLD. By contrast, QQQ, \^VIX, and \^TNX are treated as \emph{context indicators}: they enter the state representation but are not direct allocation targets. On the macro side, the current working snapshot already uses the 16-series \emph{core} preset. The optional Yahoo news and Financial PhraseBank inputs are not present in the current working tree, so news-derived processed tables are empty until those raw inputs are refreshed.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../figures/data_coverage_timeline.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../figures/normalized_assets.png}
